...
Coordinate GetPiece (Set avail)
{
        ...
        for (i = 0; i < avail.count; i++) printf ("\t[%d] (%d, %d)\n", i + 1, 
                                                                       avail.coordinate[i].x, 
                                                                       avail.coordinate[i].y); //prints all the available coordinates modified by ModifyValid()
	printf ("Enter choice: ");
        scanf ("%d", &i); //asks user to prompt for input 

	while (i < 1 || i > avail.count) // loops until prompted choice contains a valid move 
        {
                printf ("%sInvalid input.%s", RED, RESET);
                printf ("\nPlease enter a valid choice: \n");
                for (i = 0; i < avail.count; i++) printf ("\t[%d] (%d, %d)\n", i + 1, 
                                                                               avail.coordinate[i].x, 
                                                                               avail.coordinate[i].y);
                scanf ("%d", &i);
        } 
...
Coordinate GetMove (Set avail, int aTurn, Coordinate prev) // huhu do i even need this
{
        ...
        {
                if (avail.coordinate[i].x == left.x && 
                    avail.coordinate[i].y == left.y)
                         printf ("\t[%d] (%d, %d) - %sLeft (<--)%s\n",i + 1,
                                                                      avail.coordinate[i].x, 
                                                                      avail.coordinate[i].y, 
                                                                      GREEN, 
                                                                      RESET);
                if (avail.coordinate[i].x == front.x && 
                    avail.coordinate[i].y == front.y)
                         printf ("\t[%d] (%d, %d) - %sForward ( ^ )%s\n",i + 1, 
                                                                         avail.coordinate[i].x, 
                                                                         avail.coordinate[i].y, 
                                                                         GREEN, 
                                                                         RESET);
                if (avail.coordinate[i].x == right.x && 
                    avail.coordinate[i].y == right.y)
...
Set AvailablePieces (Set current, Game *game) 
{
        ...
        {
                temp = ModifyValid (game, 
                                    game->aTurn, 
                                    current.coordinate[i]); // holds a structure which contains moves which varies from left, right, front that are valid
                if (temp.count > 0) 
                holder.coordinate[holder.count++] = current.coordinate[i]; // if a piece has a count > 0, then its coordinates will be stored in holder 
        }
...
Set ModifyValid (Game *game, int aTurn, Coordinate prev)
{
	...
	for (i = 0; i < 3; i++)
	{
		if (Contains (&all.coordinate[i], game->free) == TRUE) 
                        temp.coordinate[temp.count++] = all.coordinate[i]; // if the coordinate is in the free set, then it is added to the temp set
...
int GameOver (Game *game) //return game state
{
        ...
        for (i = 0; i < game->alpha.count; i++) 
                if (Contains (&game->alpha.coordinate[i], game->Y) == TRUE) in_setE++; // checks if the current player's pieces are in the Y set (base of beta)
        for (i = 0; i < game->beta.count; i++) 
                if (Contains (&game->beta.coordinate[i], game->E) == TRUE) in_setY++; // checks if the current player's pieces are in the E set (base of alpha)
        ...
        if (game->alpha.count - in_setE == 0) 
                ...
                return BETA_WINS_ON_BASE; // if all of beta's pieces occupy alpha's base then beta wins
                 ...
                 (temp_beta.count == 0  && game->alpha.count > in_setE )) 
...
void NextPlayerMove (Coordinate prev, Game *game)
{
        ...
        {
                ALPHA_TURN;
                game->valid = AvailablePieces (game->alpha, 
                                               game); 
        } else {
                BETA_TURN;
                game->valid = AvailablePieces (game->beta, 
                                               game);
        }
        ...
        if (game->valid.count == 0)
                ...
                game->valid = ModifyValid (game, 
                                           game->aTurn, 
                                           prev); // modifies the valid set to include only the available moves

                game->next = GetMove (game->valid, 
                                      game->aTurn, 
                                      prev); // determines where the piece will be moved to

                EliminationProcess (game, 
                                    prev, 
                                    game->next, 
                                    game->aTurn);
                DisplayBoard(game);
...
void DeclareWinner(int over, Game *game)
{
        ...
        case ALPHA_WINS_ON_BASE:
                printf ("\nAll of alpha's current piece(s) are in beta's base!\n");
                Divider ();
                ALPHA_WIN;
        ...
        case BETA_WINS_ON_BASE:
                printf ("\nAll of beta's current piece(s) are in alpha's base!\n");
                Divider ();
                BETA_WIN;
